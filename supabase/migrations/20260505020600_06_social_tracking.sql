...
CREATE INDEX ON deck_votes (profile_id);
...
CREATE INDEX ON deck_vote_events (profile_id, changed_at DESC);
...
CREATE POLICY "deck_vote_reviews: read visible" ON deck_vote_reviews FOR SELECT
  USING (EXISTS (SELECT 1 FROM decks d WHERE d.id = deck_vote_reviews.deck_id AND (d.visibility_state IN ('public', 'unlisted') OR d.profile_id = current_profile_id())));
...
CREATE INDEX ON deck_vote_reviews (profile_id, created_at DESC);
...
CREATE INDEX ON deck_vote_review_comments (profile_id, created_at DESC);
...
CREATE POLICY "deck_downloads: insert own" ON deck_downloads FOR INSERT WITH CHECK (profile_id = current_profile_id());
CREATE POLICY "deck_downloads: read own" ON deck_downloads FOR SELECT USING (profile_id = current_profile_id());
...
CREATE TABLE deck_favorites (
  ...
  profile_id uuid NOT NULL CONSTRAINT deck_favorites_profile_id_fkey REFERENCES profiles(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (deck_id, profile_id)
);
ALTER TABLE deck_favorites ENABLE ROW LEVEL SECURITY;
CREATE POLICY "deck_favorites: manage own" ON deck_favorites FOR ALL USING (profile_id = current_profile_id()) WITH CHECK (profile_id = current_profile_id());
