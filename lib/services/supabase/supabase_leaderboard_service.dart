...
class SupabaseLeaderboardService extends SupabaseService {
  Future<List<Map<String, dynamic>>> fetchLeaderboard({
    ...
    if (targetLanguage != null) {
      query = query.eq('targetLanguage', targetLanguage);
    }
    final response = await query.order('drillScore', ascending: false);
  ...
  Future<Map<String, dynamic>?> fetchStreak(String userId) => guard(
    () => client.from('streaks').select().eq('userId', userId).maybeSingle(),
  );
}
