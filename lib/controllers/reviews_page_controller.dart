...
typedef ReviewItem = ({FsrsCardState fsrsState, DeckCard card});
...
class ReviewsPageController extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      for (final state in dueStates) {
        final card = _deckCardRepository.getById(state.cardId);
        if (card != null) {
          joined.add((fsrsState: state, card: card));
        }
      }
      _items = joined;
      _currentIndex = 0;
  ...
  Future<void> rateAndAdvance(int rating) async {
    final item = currentItem;
    if (item == null) return;
    try {
      final (updatedState, logEntry) =
          _fsrsService.reviewCard(item.fsrsState, rating);
      await _fsrsRepository.save(updatedState);
      await _reviewLogRepository.save(logEntry);
